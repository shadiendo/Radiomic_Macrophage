...
showThePlot <- function(group,title_name) {
  ...
  gg <- ggscatter(df_demo, x = "enrichment_value", y = "riskScore_log",
                  ...
                  # ———————————————————————————————————————————
                  palette = "jama",     # 配色方案杂志 jama,nature,lancet,jco
                  # ———————————————————————————————————————————
                  # ellipse = TRUE,  # 画椭圆
                  # mean.point = TRUE,  # 中值
                  # star.plot = TRUE   #生成星图
...
ggsave('→散点图结果汇总_tmp.pdf', plot_ret, width = 28, height = 21,dpi = 300)#保存文件
