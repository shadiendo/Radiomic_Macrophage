...
# 打开数据
# █████████████████████████████████████████████████████████████████████████████
df <- read.csv('demo_data.csv', header=T,row.names = 1)
...
# 筛出训练集、测试集数据
TCGA_LGG <- read.csv('groups/tcga_lgg.txt', header=F)
TCGA_GBM <- read.csv('groups/tcga_gbm.txt', header=F) 
RBRT_LGG <- read.csv('groups/rbrt_lgg.txt', header=F)
RBRT_GBM <- read.csv('groups/rbrt_gbm.txt', header=F)
JSPH_LGG <- read.csv('groups/jsph_lgg.txt', header=F) 
JSPH_GBM <- read.csv('groups/jsph_gbm.txt', header=F)
test_JSPH_LGG <- read.csv('groups/TEST_jsph_lgg.txt', header=F)
test_JSPH_GBM <- read.csv('groups/TEST_jsph_gbm.txt', header=F)
...
showThePlot <- function(group,thisPlotTitle,cut_off,lowrisk_color,highrisk_color) {
  ...
  legend_LowRisk = paste0('Low Risk')
  legend_HighRisk = paste0('High Risk')
  legend_HighRisk
  legend_LowRisk
